...
logFile = os.path.join(xbmcvfs.translatePath(
    addon.getAddonInfo('profile')), addonId+'.log')
...
def addLink(name, mode, logo, plot=None, url=None, idCh=None, StreamType=None):
    u = sys.argv[0] + "?name=" + urllib.parse.quote_plus(name) + \
                      "&logo=" + \
        urllib.parse.quote_plus(logo) + '&mode=' + str(mode)
    if (url):
        u += "&url=" + urllib.parse.quote_plus(url)
    if (idCh):
        u += "&idCh=" + str(idCh)
    ...
    if (addon.getSetting('api') == 'true'):
        ...
        if (not addon.getSetting('deviceId') or not addon.getSetting('DOSESSV3PRI')):
            xbmcgui.Dialog().ok(addon.getLocalizedString(
                30013), addon.getLocalizedString(30014))
            return
...
def listCh(url, idCat, StreamType, DirType):
    if (idCat != None):
        deviceIdFile = os.path.join(xbmcvfs.translatePath(
            addon.getAddonInfo('profile')), '.deviceId')
        epgFile = os.path.join(xbmcvfs.translatePath(
            addon.getAddonInfo('profile')), '.epg')
        ...
        if isdir == 0 and DirType == "6":  # All TV
            ...
            for cat in AllCats:
                if cat['StreamType'] == "LiveTV":
                    channels = digi.getChannels(
                        cat['id'], cat['StreamType'], "1")
                    for ch in channels:
        ...
        if isdir == 0:
            ...
            for cat in submenus:
                if cat['url'].startswith(url) and cat['url'] != url:
                    addDir(name=cat['name'].encode(
                        'utf8'), url=cat['url'], mode=1)
                    isdir = 1

        if isdir == 0:
            pages = digi.scrapPages(
                html=html, url=url, page_offset=addon.getSetting('titles_per_page'))
            if pages and pages != None:
                ...
                for page in pages:
                    ...
                    series = series + \
                        digi.scrapCats('series', html, page['url'])
                new_series = []
                for elem in series:
                ...
                for cat in series:
                    if cat['parent'] == url.split("?")[0] and cat['url'] != url.split("?")[0] and 'seriale' in cat['url']:
                        addDir(name=cat['name'], url=cat['url'],
                               mode=1, logo=cat['logo'])
                        isdir = 1
        ...
        if isdir == 0:
            ...
            for cat in seasons:
                if cat['parent'] == url and cat['url'] != url and 'sezon' in cat['url'] and 'sezon' not in url:
                    addDir(name=cat['name'].replace(
                        "-", " ").capitalize().encode('utf8'), url=cat['url'], mode=1)
                    isdir = 1
...
def PlayMPD(url, name, logo, idCh, StreamType=False):
    ...
    listitem.setInfo('video', {'Title': name})
    KODI_VERSION_MAJOR = int(xbmc.getInfoLabel(
    ...
    addon_log(
        "Cache subtitles enabled, downloading and converting subtitles in: " + folder)
    ...
    try:
        ...
        if len(subtitles) > 0:
            ...
            for sub in subtitles:
                addon_log("Processing subtitle language code: " +
...
try:
    ...
    StreamType = None
try:
    # 1-Movie, 2-Series, 3-Season, 4-Episode, 5-Kids, 6-All TV
...
if (deviceId and DOSESSV3PRI):
    ...
    xbmc.executebuiltin("Notification(%s,%s,%i)" %
                        (addon.getLocalizedString(30020), "", 5000))
    listCat()
